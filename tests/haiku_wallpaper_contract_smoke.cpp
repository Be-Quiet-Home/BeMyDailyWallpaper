...
main()
{
	...
	if (ValidateSingleField(
		...
		return Fail("image path field had an unexpected contract");
	}

	if (ValidateSingleField(
			message, B_BACKGROUND_MODE, B_INT32_TYPE) != B_OK) {
		return Fail("mode field had an unexpected contract");
	}

	if (ValidateSingleField(
			message, B_BACKGROUND_ORIGIN, B_POINT_TYPE) != B_OK) {
		return Fail("origin field had an unexpected contract");
	}

	if (ValidateSingleField(
			message, B_BACKGROUND_ERASE_TEXT, B_BOOL_TYPE) != B_OK) {
		return Fail("text outline field had an unexpected contract");
	}

	if (ValidateSingleField(
			message, B_BACKGROUND_WORKSPACES, B_INT32_TYPE) != B_OK) {
		return Fail("workspace field had an unexpected contract");
	}

	const char* storedImagePath = NULL;
	if (message.FindString(
			B_BACKGROUND_IMAGE, &storedImagePath) != B_OK) {
		return Fail("image path field was not readable");
	}

	if (strcmp(storedImagePath, imagePath) != 0)
		return Fail("image path field changed the path");

	int32 mode = -1;
	if (message.FindInt32(B_BACKGROUND_MODE, &mode) != B_OK)
		return Fail("mode field was not readable");

	if (mode != B_BACKGROUND_MODE_SCALED)
		return Fail("message did not request scaled placement");

	BPoint origin;
	if (message.FindPoint(B_BACKGROUND_ORIGIN, &origin) != B_OK)
		return Fail("origin field was not readable");

	if (origin.x != 0 || origin.y != 0)
		return Fail("message returned an unexpected origin");

	bool textOutline = false;
	if (message.FindBool(
			B_BACKGROUND_ERASE_TEXT, &textOutline) != B_OK) {
		return Fail("text outline field was not readable");
	}

	if (!textOutline)
		return Fail("message did not enable icon label outline");

	int32 workspaces = 0;
	if (message.FindInt32(
			B_BACKGROUND_WORKSPACES, &workspaces) != B_OK) {
		return Fail("workspace field was not readable");
	}

	if (workspaces != (int32)B_ALL_WORKSPACES)
		return Fail("message did not target all workspaces");
